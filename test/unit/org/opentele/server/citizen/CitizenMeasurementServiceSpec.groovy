...
class CitizenMeasurementServiceSpec extends Specification {
    ...
    void "can get list of raw measurements of specific type"() {
        setup:
        new MeasurementBuilder().ofType(MeasurementTypeName.BLOOD_PRESSURE).inQuestionnaire(completedQuestionnaire).build()
        new MeasurementBuilder().ofType(MeasurementTypeName.PULSE).inQuestionnaire(completedQuestionnaire).build()
        new MeasurementBuilder().ofType(MeasurementTypeName.BLOOD_PRESSURE).inQuestionnaire(completedQuestionnaire).build()

        when:
        List<Measurement> bloodPressureMeasurements = service.patientMeasurementsOfType(patient, TimeFilter.all(), MeasurementTypeName.BLOOD_PRESSURE)
        List<Measurement> pulseMeasurement = service.patientMeasurementsOfType(patient, TimeFilter.all(), MeasurementTypeName.PULSE)

        then:
        bloodPressureMeasurements.size() == 2
        bloodPressureMeasurements.findAll {m -> m.measurementType.name == MeasurementTypeName.PULSE}.size() == 0

        pulseMeasurement.size() == 1
        pulseMeasurement.findAll {m -> m.measurementType.name == MeasurementTypeName.BLOOD_PRESSURE}.size() == 0
    }

    void "list of raw measurements of specific type is sorted newest first"() {
        setup:
        new MeasurementBuilder().ofType(MeasurementTypeName.BLOOD_PRESSURE).atTime(2014, 1, 1, 9, 40).inQuestionnaire(completedQuestionnaire).build()
        new MeasurementBuilder().ofType(MeasurementTypeName.BLOOD_PRESSURE).atTime(2014, 1, 1, 12, 40).inQuestionnaire(completedQuestionnaire).build()
        new MeasurementBuilder().ofType(MeasurementTypeName.BLOOD_PRESSURE).atTime(2014, 1, 1, 11, 40).inQuestionnaire(completedQuestionnaire).build()

        when:
        List<Measurement> measurements = service.patientMeasurementsOfType(patient, TimeFilter.all(), MeasurementTypeName.BLOOD_PRESSURE)
    ...
    void "if no raw measurements of specific type is found an empty list is returned"() {
        when:
        def measurements = service.patientMeasurementsOfType(patient, TimeFilter.all(), MeasurementTypeName.BLOOD_PRESSURE)
    ...
    void 'can give all CGM measurements as graph data'() {
        ...
        measurement.addToContinuousBloodSugarEvents(new CoulometerReadingEvent(recordNumber: 2004, time: new Date(now), glucoseValueInmmolPerl: 1))
    ...
    void 'Can handle CGM measurement without any events'() {
        ...
        when:
        List<GraphData> graphDataList = service.dataForGraphs(patient, TimeFilter.all())
